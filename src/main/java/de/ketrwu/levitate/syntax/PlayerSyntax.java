...
public class PlayerSyntax implements SyntaxHandler {
    ...
    public void check(CommandSender sender, String parameter, String passed) throws SyntaxResponseException, CommandSyntaxException {
        if (parameter.equalsIgnoreCase("online") == false && parameter.equalsIgnoreCase("offline") == false && parameter.equals("") == false)
            throw new CommandSyntaxException(new MessageBuilder(Message.PLAYERSYNTAX_PARAMETER_MALFORMED, TextMode.COLOR).replace("%parameter%", parameter));
        ...
        if (passed.length() > 16 && passed.contains("-")) {
            ...
            try {
                ...
                if (p == null)
                    throw new SyntaxResponseException(new MessageBuilder(Message.PLAYERSYNTAX_PLAYER_NOT_FOUND, TextMode.COLOR, replaces));
                if (p.getName() == null || p.getName().equalsIgnoreCase(passed))
                    throw new SyntaxResponseException(new MessageBuilder(Message.PLAYERSYNTAX_PLAYER_NOT_FOUND, TextMode.COLOR, replaces));
        ...
        if (parameter.equalsIgnoreCase("online")) {
            if (!p.isOnline())
                throw new SyntaxResponseException(new MessageBuilder(Message.PLAYERSYNTAX_PLAYER_OFFLINE, TextMode.COLOR, replaces));
        } else if (parameter.equalsIgnoreCase("offline")) {
            if (p.isOnline())
                throw new SyntaxResponseException(new MessageBuilder(Message.PLAYERSYNTAX_PLAYER_ONLINE, TextMode.COLOR, replaces));
        }
    }
