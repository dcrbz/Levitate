...
public class IntegerSyntax implements SyntaxHandler {
    ...
    public void check(CommandSender sender, String parameter, String passed) throws CommandSyntaxException, SyntaxResponseException {
        ...
        if (!isInt(passed))
            throw new SyntaxResponseException(new MessageBuilder(Message.INTEGERSYNTAX_HAS_NO_INTEGER, TextMode.COLOR, replaces));
        int passedInt = Integer.parseInt(passed);
        if (parameter.equals("+")) {
            if (passedInt < 0 && passedInt != 0)
                throw new SyntaxResponseException(new MessageBuilder(Message.INTEGERSYNTAX_HAS_TO_BE_POSITIVE_ZERO, TextMode.COLOR, replaces));
        } else if (parameter.equals("-")) {
            if (passedInt > 0 && passedInt != 0)
                throw new SyntaxResponseException(new MessageBuilder(Message.INTEGERSYNTAX_HAS_TO_BE_NEGATIVE_ZERO, TextMode.COLOR, replaces));
        } else if (parameter.contains("-")) {
            replaces.put("%parameter%", parameter);
            if (!parameter.matches("(-?[0-9]+)-(-?[0-9]+)"))
                throw new CommandSyntaxException(new MessageBuilder(Message.INTEGERSYNTAX_PARAMETER_MALFORMED, TextMode.COLOR, replaces));
            if (parameter.startsWith("-")) {
                ...
                if (a > b)
                    throw new SyntaxResponseException(new MessageBuilder(Message.INTEGERSYNTAX_PARAMETER_MALFORMED_2, TextMode.COLOR, replaces));
                if (passedInt < a || passedInt > b)
                    throw new SyntaxResponseException(new MessageBuilder(Message.INTEGERSYNTAX_HAS_TO_BE_INBETWEEN, TextMode.COLOR, replaces));
                return;
            } else {
