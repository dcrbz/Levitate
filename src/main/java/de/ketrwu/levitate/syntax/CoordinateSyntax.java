...
public class CoordinateSyntax implements SyntaxHandler {
    ...
    public void check(CommandSender sender, String parameter, String passed) throws SyntaxResponseException, CommandSyntaxException {
        ...
        if (!(sender instanceof Player))
            throw new CommandSyntaxException(new MessageBuilder(Message.COORDSYNTAX_CONSOLE_COMMAND, TextMode.COLOR, replaces));
        ...
        switch (parameter.trim().toLowerCase()) {
            ...
            case "yaw":
            case "pitch":
                if (!isInt(passed) && !isDouble(passed) && !isFloat(passed))
                    throw new SyntaxResponseException(new MessageBuilder(Message.COORDSYNTAX_HAS_TO_BE_NUMBER, TextMode.COLOR, replaces));
                break;
            default:
                throw new CommandSyntaxException(new MessageBuilder(Message.COORDSYNTAX_PARAMETER_MALFORMED, TextMode.COLOR, replaces));
        }
