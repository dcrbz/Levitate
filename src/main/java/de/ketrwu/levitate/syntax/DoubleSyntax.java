...
public class DoubleSyntax implements SyntaxHandler {
	...
	public void check(CommandSender sender, String parameter, String passed) throws CommandSyntaxException, SyntaxResponseException {
		...
		if(!isDouble(passed)) throw new SyntaxResponseException(new MessageBuilder(Message.DOUBLESYNTAX_HAS_NO_DOUBLE, TextMode.COLOR, replaces));
		double parsedDouble = Double.parseDouble(passed);
		if(parameter.equals("+")) {
			if(parsedDouble < 0 && parsedDouble != 0) throw new SyntaxResponseException(new MessageBuilder(Message.DOUBLESYNTAX_HAS_TO_BE_POSITIVE_ZERO, TextMode.COLOR, replaces));
		} else if(parameter.equals("-")) {
			if(parsedDouble > 0 && parsedDouble != 0) throw new SyntaxResponseException(new MessageBuilder(Message.DOUBLESYNTAX_HAS_TO_BE_NEGATIVE_ZERO, TextMode.COLOR, replaces));
		} else if(parameter.contains("-")) {
			replaces.put("%parameter%", parameter);
			if(!parameter.matches("(-?[0-9]+)-(-?[0-9]+)")) throw new CommandSyntaxException(new MessageBuilder(Message.DOUBLESYNTAX_PARAMETER_MALFORMED, TextMode.COLOR, replaces));
			if(parameter.startsWith("-")) {
				...
				if(a > b) throw new SyntaxResponseException(new MessageBuilder(Message.DOUBLESYNTAX_PARAMETER_MALFORMED_2, TextMode.COLOR, replaces));
				if(parsedDouble < a || parsedDouble > b) throw new SyntaxResponseException(new MessageBuilder(Message.DOUBLESYNTAX_HAS_TO_BE_INBETWEEN, TextMode.COLOR, replaces));
				return;
			} else {
